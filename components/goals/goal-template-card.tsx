...
export function GoalTemplateCard({ title, description, category, icon, duration, durationLabel, actionLabel, action }: { title: string; description?: string | null; category: GoalCategory; icon?: string | null; duration: number; durationLabel: string; actionLabel: string; action: ReactNode }) {
  ...
  return (
    <Card className="space-y-4 hover:shadow-[0_22px_52px_rgba(31,41,51,0.10)]">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="font-extrabold tracking-[-0.01em] text-text-primary">{title}</h3>
          <p className="mt-1 text-sm leading-6 text-text-secondary">{description}</p>
          <p className="mt-3 inline-flex rounded-pill bg-app-surface px-3 py-1 text-xs font-extrabold text-text-soft">{duration} {durationLabel}</p>
        </div>
      </div>
